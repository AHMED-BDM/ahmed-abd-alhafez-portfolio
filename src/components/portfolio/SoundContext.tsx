...
type Ctx = {
  enabled: boolean;
  toggle: () => void;
  play: (kind: "hover" | "click" | "open" | "rumble" | "whisper" | "curse") => void;
};
...
const useSynth = () => {
  ...
  const tone = (freq: number, dur: number, type: OscillatorType = "sine", vol = 0.04) => {
    const ac = ensure();
  ...
  const noise = (dur: number, vol = 0.05, lp = 800) => {
    const ac = ensure();
...
export const SoundProvider = ({ children }: { children: ReactNode }) => {
  ...
  const play = useCallback((kind: Parameters<Ctx["play"]>[0]) => {
    if (!enabled) return;
    switch (kind) {
      case "hover": synth.tone(880, 0.12, "sine", 0.02); break;
      case "click": synth.tone(660, 0.18, "triangle", 0.05); setTimeout(() => synth.tone(990, 0.15, "sine", 0.04), 60); break;
      case "open": synth.noise(1.6, 0.08, 500); setTimeout(() => synth.tone(196, 0.8, "sawtooth", 0.04), 200); break;
      case "rumble": synth.noise(2, 0.1, 200); break;
      case "whisper": synth.noise(2.4, 0.04, 1500); break;
      case "curse": synth.tone(110, 1.2, "sawtooth", 0.06); synth.noise(1.2, 0.06, 400); break;
    }
  }, [enabled, synth]);
